...
use crate::fusion::wayland::{WaylandServer, WaylandError};
use crate::graphics::Display;
use crate::graphics::vesa::VesaDisplay;
use crate::terminal::Terminal;
use crate::utils::pointer;

use alloy_os_cosmos_de::{CosmosBootProfile, CosmosDe, CosmosBootstrapReport};
...
const WAYLAND_CLIENT_ID: ClientId = ClientId::new(64);
...
fn activate_shell_app<B: DisplayBackend>(
    ...
    *runtime.binding_mut(app) = None;
    ...
    let created = spawn_toolbox_window(
        wm, server, app, &state,
        workspace_width, workspace_height, terminal_width, terminal_height,
    )?;
...
pub fn run(display: VesaDisplay, mode: BootUiMode) -> Result<(), DisplayServerBootError> {
    match mode {
        BootUiMode::Cosmos => {
            let bootstrap_report = CosmosDe::new(CosmosBootProfile::cosmos_default()).bootstrap_report();
            run_with_bootstrap(display, mode, bootstrap_report)
        }
        BootUiMode::IcedPrimary => run_iced_primary(display),
    }
}

/// Display server runtime entry point with Cosmos DE bootstrap integration.
pub fn run_with_bootstrap(
    display: VesaDisplay,
    mode: BootUiMode,
    bootstrap_report: CosmosBootstrapReport,
) -> Result<(), DisplayServerBootError> {
    match mode {
        BootUiMode::Cosmos => run_cosmos(display, bootstrap_report),
        BootUiMode::IcedPrimary => run_iced_primary(display),
    }
}

/// Cosmos DE runtime with Wayland compositor bridge and session boundary.
fn run_cosmos(display: VesaDisplay, bootstrap_report: CosmosBootstrapReport) -> Result<(), DisplayServerBootError> {
    serial_log(b"[DisplayServer] Bootstrapping Cosmos DE runtime with Wayland bridge\n\0");
    let (display_width, display_height) = display.get_resolution();

    let session_boundary = bootstrap_report.boundary;
    serial_log(b"[DisplayServer] Session boundary: input_owner=");
    serial_log_u32(session_boundary.input_owner.0);
    serial_log(b" shell_owner=");
    serial_log_u32(session_boundary.shell_owner.0);
    serial_log(b" compositor_owner=");
    serial_log_u32(session_boundary.compositor_owner.0);
    serial_log(b"\n\0");

    let backend = FusionDisplayBackend::new(display);
    let mut server = DisplayServer::new(backend);

    alloy_os_display::server::set_session_boundary(&mut server, session_boundary);
...
// Initialize Wayland server for userland compositor clients
     let mut wayland_server = WaylandServer::new();
     if let Err(e) = wayland_server.init() {
         serial_log(b"[DisplayServer] Warning: Wayland server init failed: ");
         serial_log(e.as_bytes());
         serial_log(b"\n\0");
     }
    ...
    if mouse_ready {
        ...
        serial_log(b"[DisplayServer] Mouse unavailable; cursor hidden\n\0");
    }
    ...
    serial_log(b"[DisplayServer] Cosmos DE ready with Wayland bridge\n\0");
    ...
    loop {
        ...
        // Composite Wayland surfaces
        if wayland_server.is_listening() {
            wayland_server.composite_frame();
        }
            ...
            match event {
                DisplayEvent::FocusChanged {
                    ...
                    match runtime.app_for_surface(surface_id) {
                        Some(ShellApp::Terminal) => serial_log(b"[DisplayServer] Focus -> terminal\n\0"),
                        Some(ShellApp::Settings) => serial_log(b"[DisplayServer] Focus -> settings\n\0"),
                        Some(ShellApp::FileExplorer) => serial_log(b"[DisplayServer] Focus -> file explorer\n\0"),
                        Some(ShellApp::TextEditor) => serial_log(b"[DisplayServer] Focus -> text editor\n\0"),
                        Some(ShellApp::Calculator) => serial_log(b"[DisplayServer] Focus -> calculator\n\0"),
                        None => serial_log(b"[DisplayServer] Focus -> unmanaged surface\n\0"),
                    }
                }
                ...
                DisplayEvent::SurfaceDestroyed { surface_id } => {
                    if let Some(app) = runtime.app_for_surface(surface_id) {
                        *runtime.binding_mut(app) = None;
                        shell.clear_binding(app);
                        match app {
                            ShellApp::Terminal => serial_log(b"[DisplayServer] Terminal destroyed\n\0"),
                            ShellApp::Settings => serial_log(b"[DisplayServer] Settings destroyed\n\0"),
                            ShellApp::FileExplorer => serial_log(b"[DisplayServer] File explorer destroyed\n\0"),
                            ShellApp::TextEditor => serial_log(b"[DisplayServer] Text editor destroyed\n\0"),
                            ShellApp::Calculator => serial_log(b"[DisplayServer] Calculator destroyed\n\0"),
                        }
                    }
                }
                DisplayEvent::SessionBoundaryChanged { session_id: _, boundary } => {
                    serial_log(b"[DisplayServer] Session boundary changed\n\0");
                    alloy_os_display::server::set_session_boundary(&mut server, boundary);
                }
                _ => {}
            }
        }
    ...
    if diagnostics.dropped_events > 0 {
        serial_log(b"[DisplayServer] Warning: event queue overflow\n\0");
    }
    if diagnostics.backend_errors > 0 {
        serial_log(b"[DisplayServer] Warning: backend errors\n\0");
    }
...
mod tests {
    ...
    #[test]
    fn test_wayland_client_id_constant() {
        assert_eq!(WAYLAND_CLIENT_ID.0, 64);
    }
}