    ...
    fn execute(&self, args: &[&str], registry: &CommandRegistry) -> Result<(), &str> {
        ...
        if let Some(command_name) = args.first() {
            if let Some(command) = registry.get(command_name) {
                colors::print_info(&alloc::format!("{:<8} - {}", command_name, command.help()));
                return Ok(());
            }
    ...
    fn execute(&self, _args: &[&str], _registry: &CommandRegistry) -> Result<(), &str> {
        ...
        // Get PMM statistics
        {
            ...
            crate::terminal::println_cstr(
                &total_frames_str[format::trim_leading_spaces(&total_frames_str)..],
            );

            crate::VgaText::print_bytes(b"  Used frames:      ");
            crate::terminal::println_cstr(
                &used_frames_str[format::trim_leading_spaces(&used_frames_str)..],
            );

            crate::VgaText::print_bytes(b"  Free frames:      ");
            crate::terminal::println_cstr(
                &free_frames_str[format::trim_leading_spaces(&free_frames_str)..],
            );
        }
    ...
    fn execute(&self, _args: &[&str], _registry: &CommandRegistry) -> Result<(), &str> {
        ...
        let features = alloy_kernel_hal::cpu_get_features();
            ...
            if features & CPU_FEATURE_APIC != 0 {
                crate::VgaText::println_bytes(
                    b"  [x] APIC  - Advanced Programmable Interrupt Controller",
                );
            }
