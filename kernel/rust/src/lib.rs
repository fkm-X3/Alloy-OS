...
pub extern "C" fn rust_main() {
    unsafe {
        ffi::serial_print(b"[Rust] Kernel entry - initializing allocators\n\0".as_ptr());
    }
    
    // Test allocations to verify memory system
    let test_vec: Vec<u32> = vec![1, 2, 3, 4, 5];
    let test_string = String::from("Alloy OS Terminal");
    
    unsafe {
        ffi::serial_print(b"[Rust] Memory allocators verified\n\0".as_ptr());
